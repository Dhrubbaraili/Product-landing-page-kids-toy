...
import { getMissingOrderConfig, getOrderConfigPresence, getOrderRuntimeConfig, type OrderRuntimeConfig } from '@/lib/order-config';
...
function getServiceAccountCredentials(config: OrderRuntimeConfig) {
  const encodedCredentials = config.googleServiceAccountJsonBase64?.trim();
  ...
  const configuredPath = config.googleServiceAccountJsonPath;
  if (configuredPath) {
  ...
  return validateServiceAccountCredentials({ client_email: config.googleServiceAccountEmail, private_key: config.googlePrivateKey });
}

async function getSheetsClient(config: OrderRuntimeConfig) {
  const auth = new google.auth.GoogleAuth({ credentials: getServiceAccountCredentials(config), scopes: ['https://www.googleapis.com/auth/spreadsheets'] });
  await auth.getAccessToken();
  return google.sheets({ version: 'v4', auth });
}

async function ensurePremiumSheetLayout(sheets: ReturnType<typeof google.sheets>, config: OrderRuntimeConfig) {
  const spreadsheetId = config.googleSheetId!;
  const configuredTab = config.googleSheetTabName!.trim();
...
export async function POST(request: Request) {
  const config = getOrderRuntimeConfig();
  console.info('[order] environment presence', getOrderConfigPresence(config));
  ...
  const missing = getMissingOrderConfig(config);
  ...
  try {
    ...
    let tab: string;
    try { tab = await ensurePremiumSheetLayout(sheets, config); } catch (error) { if (error instanceof Error && error.message.includes('Google Sheet tab')) stage = 'TAB_NOT_FOUND'; throw error; }
    stage = 'SHEET_WRITE';
    await sheets.spreadsheets.values.append({ spreadsheetId: config.googleSheetId!, range: sheetRange(tab, 'A:M'), valueInputOption: 'USER_ENTERED', requestBody: { values: [[id, date, name, phone, email, location, productName, quantityNumber, priceNumber, totalNumber, payment, status, '']] } });
    const transporter = nodemailer.createTransport({ host: config.smtpHost, port: Number(config.smtpPort), secure: Number(config.smtpPort) === 465, auth: { user: config.smtpUser, pass: config.smtpPass } });
    stage = 'SMTP_LOGIN';
